...
final class DispatchTabBarController: UITabBarController, RoleCoordinator {
    ...
    /// `loadListScreenFactory?(self.role)` to back the Dispatch Loads tab.
    /// `nil` fallback preserves the v1.1 placeholder for any test that
    /// constructs the tab bar without injecting the factory.
